...
namespace MCM.UI.ViewModelWrappers
{
    ...
    public class SelectorVMWrapper : ViewModelWrapper
    {
        ...
        public SelectorVMWrapper(object? @object) : base(@object ?? MCMSelectorVM<DropdownSelectorItemVM>.Empty)
        {
            if (@object is null)
                @object = MCMSelectorVM<DropdownSelectorItemVM>.Empty;
            else if (@object is IEnumerable<object> enumerable)
                @object = new DropdownMCM(enumerable, new SelectedIndexWrapper(@object).SelectedIndex);
