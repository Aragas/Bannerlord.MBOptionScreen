...
namespace MCM.UI.Dropdown
{
    internal sealed class CheckboxDropdownSelectorItemVM<T> : DropdownSelectorItemVM<T> where T : class
    {
        ...
        [DataSourceProperty]
        public override ViewModel? Hint { get; }
        ...
        public CheckboxDropdownSelectorItemVM(T @object) : base(@object)
        {
            ...
            Hint = AccessTools2.Property(typeof(T), "HintText")?.GetValue(@object) is string hintText ? HintViewModelUtils.Create(new TextObject(hintText)) : HintViewModelUtils.Create();
        }
    }
}*/