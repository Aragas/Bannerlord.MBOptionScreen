...
namespace MCM.Implementation.ModLib
{
    internal sealed class MCMModLibSettings : AttributeGlobalSettings<MCMModLibSettings>
    {
        ...
        public override string DisplayName => TextObjectHelper.Create("{=MCMModLibSettings_Name}MCM ModLib Impl. {VERSION}", new Dictionary<string, TextObject>
        {
            { "VERSION", TextObjectHelper.Create(typeof(MCMModLibSettings).Assembly.GetName().Version.ToString(3)) }
        }).ToString();
